...
def main():
    ...
    solar_interpolators = build_interpolators(
        angle, azimuth_grid, elevation_grid, solar_data
    )
    exposed_area = solar_exposed_area(
        0.0, azimuth_reference, elevation_reference, solar_interpolators
    )
    ...
    temperature = propagate_temperature(
        T0, exposed_area, LOS_reference, P_comm, cellInstd, dt
    )
    ...
    power_interpolator = build_power_interpolator(
        T_grid, A_grid, I_grid, V_data
    )

    # Cell/panel voltage
    V_sol = power_cell_voltage(
        LOS_reference, temperature, exposed_area, Isetpt, power_interpolator
    )
