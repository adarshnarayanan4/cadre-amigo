...
def sun_position_eci(times, LD=0.0):
    n = len(times)
...
# CADRE Sun_LOS
def sun_line_of_sight(states, r_e2s_I):
    n = states.shape[0]
    LOS = np.zeros(n)
...
def sun_position_body(O_BI, r_e2s_I):
    n = r_e2s_I.shape[0]
    r_e2s_B = np.zeros((n, 3))

    for i in range(n):
        r_e2s_B[i] = O_BI[i] @ r_e2s_I[i]
...
def sun_position_spherical(r_e2s_B):
    n = r_e2s_B.shape[0]
...
def main():
    ...
    # Sun calculations
    r_e2s_I = sun_position_eci(times, LD=0.0)
    LOS = sun_line_of_sight(states, r_e2s_I)
    r_e2s_B = sun_position_body(O_BI, r_e2s_I)
    azimuth, elevation = sun_position_spherical(r_e2s_B)

    # Sanity checks
    sun_norms = np.linalg.norm(r_e2s_I, axis=1)

    print()
    print("=" * 70)
    print("CADRE SUN REFERENCE")
    print("=" * 70)
    ...
    print()
    print("Minimum Sun-vector magnitude:")
    print(np.min(sun_norms))

    print()
    print("Maximum Sun-vector magnitude:")
    print(np.max(sun_norms))
    ...
    print()
    print("Fraction of orbit in full sunlight:")
    print(np.mean(LOS == 1.0))

    print()
    print("Fraction of orbit in full eclipse:")
    print(np.mean(LOS == 0.0))
    ...
    # LOS plot
    plt.figure()
    plt.plot(times / 60.0, LOS)
    plt.xlabel("Time [min]")
    plt.ylabel("Line of Sight")
    plt.title("CADRE Sun Line of Sight")
    plt.grid()

    # Sun angles plot
    plt.figure()
    plt.plot(times / 60.0, azimuth, label="Azimuth")
    plt.plot(times / 60.0, elevation, label="Elevation")
    plt.xlabel("Time [min]")
    plt.ylabel("Angle [rad]")
    plt.title("CADRE Sun Position in Body Frame")
    plt.legend()
    plt.grid()
